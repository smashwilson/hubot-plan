...
class Proposal {
  ...
  yes(invitee) {
    this.accepted.set(invitee.getKey(), invitee);
  }

  no(invitee) {
    this.accepted.delete(invitee.getKey());
  }

  isAttending(invitee) {
    return this.accepted.has(invitee.getKey());
  }
...
class Event {
  ...
  finalize(index) {
    if (index === undefined || index === null) {
      ...
      if (keys.length > 1) {
        throw buildMultipleProposalsError({
          eventID: this.id,
          eventName: this.name,
  ...
  presentWith(presenter) {
    return this.isFinalized()
      ? presenter.presentFinalized(this)
      : presenter.presentUnfinalized(this);
  }
  ...
  static deserialize(payload) {
    ...
    e.proposals = payload.proposals.map(p =>
      p !== null ? Proposal.deserialize(p) : undefined
    );
