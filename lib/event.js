...
class Event {
  ...
  renderICalOn (robot, builder) {
    const attendeeFrom = (status, invitee) => {
      const uid = invitee.uid || invitee.name
      const user = robot.brain.users()[uid]
    ...
    const eventFrom = (transp, status, proposal) => {
      ...
      for (const respondant of this.responses.values()) {
        if (!proposal.isAttending(respondant)) {
          attendees.push(attendeeFrom('DECLINED', respondant))
        }
      }
    ...
    if (this.isFinalized()) {
      ...
      builder.events.push(
        ...this.proposals.filter(Boolean).map(eventFrom.bind(this, 'TRANSPARENT', 'TENTATIVE'))
      )
    }
  }
