...
module.exports = {
  ...
  builder(yargs) {
    return yargs
      ...
      .option("id", {
        describe:
          "Unique string used to identify this event in further commands.",
        string: true,
      })
      .option("propose", {
        describe:
          "Propose a *possible* date for this event. May be given multiple times.",
        array: true,
        string: true,
      })
      .option("invite", {
      ...
      .option("at", {
        describe:
          "Name the already-finalized date for this event. Cannot be used with `--propose`.",
        string: true,
      })
      .option("ping", {
  ...
  handler(context, argv) {
    const evt = context.store.getEventStore().create(argv.id, argv.name);
