...
class AttachmentPresenter extends Presenter {
  ...
  showInvitee(invitee) {
    return this.ping
      ? invitee.notify(this.userSource)
      : invitee.mention(this.userSource);
  }
  ...
  finalizedDateField(evt) {
    ...
    return {
      ...
      value: `${proposal
        .getTimespan()
        .renderRange()} _${proposal.startDate().from(this.now)}_`,
    };
  }
  ...
  inviteeConfirmationField(evt) {
    ...
    const proposal = evt.finalProposal();
    const value =
      `_Attendees (${proposal.yesCount()} confirmed)_\n` +
      Array.from(evt.invitees.values(), invitee => {
        let str = "";
        if (!evt.responses.has(invitee.getKey())) {
          str += ":grey_question:";
