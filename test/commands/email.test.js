...
describe("event email", function () {
  ...
  it("associates a new email address", async function () {
    ...
    assert.strictEqual(
      bot.response(),
      "Email address user0+added@gmail.com added.\n:slack: :star: _*user0+slack@gmail.com*_, user0+added@gmail.com"
    );
  });

  it("associates a new default email address", async function () {
    await bot.say("u0", "hubot: event email --default user0+added@gmail.com");
    assert.strictEqual(
      bot.response(),
      "Email address user0+added@gmail.com added and set as your default.\n:slack: _user0+slack@gmail.com_, :star: *user0+added@gmail.com*"
    );
  });
  ...
  it("removes an existing email address", async function () {
    ...
    await bot.say("u0", "hubot: event email --delete user0+manual0@gmail.com");
    assert.strictEqual(
      bot.response(),
      "Email address user0+manual0@gmail.com removed.\n" +
        ":slack: _user0+slack@gmail.com_, :star: *user0+manual1@gmail.com*"
    );
  });
});
