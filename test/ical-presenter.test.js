...
describe('ICalPresenter', function () {
  ...
  describe('with an unfinalized event', function () {
    ...
    it('includes the attendees from each proposed date', function () {
      ...
      assert.lengthOf(
        ical.match(/^ATTENDEE;ROLE=REQ-PARTICIPANT;PARTSTAT=ACCEPTED;CN=user0:MAILTO:user0@gmail\.com$/mg),
        2
      )
      assert.lengthOf(
        ical.match(/^ATTENDEE;ROLE=REQ-PARTICIPANT;PARTSTAT=DECLINED;CN=user0:MAILTO:user0@gmail\.com$/mg),
        1
      )

      assert.lengthOf(
        ical.match(/^ATTENDEE;ROLE=REQ-PARTICIPANT;PARTSTAT=ACCEPTED;CN=user1:MAILTO:user1@gmail\.com$/mg),
        2
      )
      assert.lengthOf(
        ical.match(/^ATTENDEE;ROLE=REQ-PARTICIPANT;PARTSTAT=DECLINED;CN=user1:MAILTO:user1@gmail\.com$/mg),
        1
      )

      assert.isNull(ical.match(/CN=user2/mg))
    })
  })
